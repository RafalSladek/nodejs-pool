...
export ROOT_SQL_PASS=$ROOT_SQL_PASS
CURUSER=$(whoami)
...
cd ~
git clone https://github.com/rafalsladek/nodejs-pool.git  # Change this depending on how the deployment goes.
cd /usr/src/gtest
sudo cmake .
sudo make
...
curl https://raw.githubusercontent.com/rafalsladek/nodejs-pool/master/deployment/monero_daemon.patch | sudo git apply -v
sudo make -j$(nproc)
...
cd ~
git clone https://github.com/rafalsladek/poolui.git
cd poolui
npm install
